<template>
  <main class="flex-1 p-6 lg:p-10">
    <h2 class="text-4xl font-bold text-gray-900 mb-6">Upcoming Meetings</h2>

    <p class="text-gray-600 mb-8 max-w-2xl">
      Toggle the switch to schedule the
      <span class="font-bold text-gray-800">Recall.ai notetaker bot</span>
      to attend.
    </p>
    ...
    <div class="space-y-4">
      ...
      >
        ...
        <div class="flex items-center space-x-3 flex-shrink-0">
          <span class="text-sm font-medium text-gray-700">Notetaker:</span>
          <div class="toggle-switch-container">
            <input
              type="checkbox"
              class="hidden toggle-checkbox"
              :id="'toggle-' + meeting.id"
              v-model="meeting.notetakerEnabled"
              @change="toggleNotetaker(meeting)"
            />
            <label :for="'toggle-' + meeting.id" class="toggle-label"></label>
          </div>
        </div>
      </div>
    </div>
